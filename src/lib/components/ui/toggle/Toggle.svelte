<script lang="ts">
	...
	const toggleVariants = cva(
		'inline-flex items-center rounded-md text-sm font-medium transition-colors data-[state=on]:bg-accent data-[state=on]:text-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 ring-offset-background hover:bg-muted hover:text-muted-foreground',
		{
			variants: {
				variant: {
					default: 'bg-transparent justify-center',
					outline:
						'bg-transparent border border-input hover:bg-accent hover:text-accent-foreground justify-center',
					nocenter: 'bg-background hover:bg-accent data-[state=on]:bg-primarylite'
				},
				size: {
					default: 'h-10 px-3',
					sm: 'h-9 px-2.5',
